...
const COURSES = [
  ...
  { key: 'hudson_mills_original_24', label: 'Hudson Mills Original (24)', holes: 24 },
]
const COURSE_CENTERS = {
  ...
  hudson_mills_original_24: { lat: 42.37374, lng: -83.91052 },
}
const COURSE_JSON_NAME = {
  ...
  hudson_mills_original_24: 'Hudson Mills Original (24)',
}
    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: H }}>
      <defs>
        <linearGradient id="gl" x1="0" x2="0" y1="0" y2="1">
          <stop offset="0%" stopColor="#c4a890" stopOpacity="0.35"/>
          <stop offset="100%" stopColor="#c4a890" stopOpacity="0.02"/>
        </linearGradient>
      ...
      {/* Lines */}
      <path d={leftPath}   fill="none" stroke="#c4a890" strokeWidth="1.5" strokeLinejoin="round" opacity="0.7"/>
      ...
      <circle cx={toX(100)} cy={toY(profiles.center_line[profiles.center_line.length-1])} r="4" fill="#c4a890" stroke="#000" strokeWidth="1"/>
    </svg>
  )
}
...
export default function CourseTab({
  ...
  useEffect(() => {
    ...
    markersRef.current.push(new gmaps.Marker({
      ...
      zIndex: 10,
      icon: { path: gmaps.SymbolPath.CIRCLE, scale: 11, fillColor: '#c4a890', fillOpacity: 1, strokeColor: '#000', strokeWeight: 2 },
          ...
          <div className="broadcast-card p-3 bg-gray-950">
            <div className="text-xs text-broadcast-cyan mb-1 flex justify-between">
              ...
              <span className="flex gap-3 text-[10px]">
                <span className="text-[#c4a890]">▬ Left</span>
